...
module.exports = function myBasicHook(sails) {
  return {
    async initialize(cb) {
      sails.config.customLogger.log('info', 'Initializing BasicHook...', null, 'message');
      try {
        const clamdscanConfig = {};
        if (process.env.CLAM_SOCKET) {
          sails.config.customLogger.log('info', 'Using CLAM_SOCKET for clamscan configuration', null, 'message');
          clamdscanConfig.socket = process.env.CLAM_SOCKET;
        }
        if (process.env.CLAM_HOST && !process.env.CLAM_SOCKET) {
          sails.config.customLogger.log('info', 'Using CLAM_HOST for clamscan configuration', null, 'message');
        ...
        sails.config.customLogger.log('info', 'clamscan initialized successfully', null, 'server-action');
      } catch (error) {
        sails.config.customLogger.log('error', "Error initializing clamscan: ", error, null, 'server-action');
      }
      try {
        sails.config.customLogger.log('info', 'Starting cron initialization', null, 'message');
        await sails.config.startCron();
        sails.config.customLogger.log('info', 'Cron initialized successfully', null, 'server-action');
      } catch (error) {
        sails.config.customLogger.log('error', "Error initializing cron", error,  'server-action');
      }
      sails.config.customLogger.log('info', 'BasicHook initialization completed' , null, 'server-action');
      return cb();
    },
  };
};
