...
module.exports = {
  ...
  async beforeCreate(consultation, cb) {
    if (
      ...
      if (defaultQueue) {
        sails.config.customLogger.log('verbose', `Assigning the default queue to the consultation as no queue is set defaultQueueId ${defaultQueue?.id}`, null, 'message', null);
  ...
  async afterCreate(consultation, proceed) {
    ...
    sails.config.customLogger.log('info', `New consultation broadcast consultationId ${consultation.id}`, null, 'server-action', null);
    return proceed();
  },

  async beforeDestroy(criteria, proceed) {
    sails.config.customLogger.log('verbose', `Deleting consultation with criteria ${criteria}`, null, 'message', null);
    ...
    if (consultation.invitationToken) {
      ...
      sails.config.customLogger.log('info', `Public invite status updated to SENT for consultation ${consultation?.id}`, null, 'server-action', null);
    }
    ...
    sails.config.customLogger.log('info', `Consultation with id ${consultation.id} cancellation broadcast`, null, 'server-action', null);
    return proceed();
  },
  ...
  async broadcastNewConsultation(consultation) {
    ...
    participants.forEach((participant) => {
      ...
      sails.config.customLogger.log('info', `Broadcast new consultation ${consultation.id} to participant ${participant} `, null, 'message', null);
    });
  ...
  async getAnonymousDetails(consultation) {
    ...
    if (consultation.invite) {
      ...
      try {
        ...
        sails.config.customLogger.log('error', `Error finding invite`, { error: error?.message || error }, 'server-action', null);
      }
    }
    ...
    try {
      ...
      sails.config.customLogger.log('info', `Anonymous consultation details computed for consultation id ${consultation.id}`, null,'message', null);
    } catch (error) {
      sails.config.customLogger.log('error', `Error counting messages for consultation id ${consultation.id}`, { error: error?.message || error }, 'server-action', null);
    }

    sails.config.customLogger.log('verbose', `Anonymous consultation created for consultation id ${consultation.id}`, null,'server-action', null);
  ...
  async sendConsultationClosed(consultation) {
    ...
    participants.forEach((participant) => {
      ...
      sails.config.customLogger.log('info', `Broadcast consultationClosed ${consultation.id} to participant ${participant}`, null, 'server-action', null);
    });
  },
  ...
  async closeConsultation(consultation) {
    ...
    if (consultation.invitationToken) {
      try {
        ...
        sails.config.customLogger.log('error', `Error destroying invite for consultation id ${consultation.id}`, { error: error?.message || error }, 'server-action', null);
      }
    }
    ...
    try {
      ...
      try {
        ...
        sails.config.customLogger.log('info', `Anonymous calls saved for consultation id ${consultation.id}`, null, 'server-action', null);
      } catch (error) {
        sails.config.customLogger.log('error', `Error creating anonymous calls for consultation id ${consultation.id}`, { error: error?.message || error }, 'server-action', null);
      }
    } catch (error) {
      sails.config.customLogger.log('error', `Error finding call messages for consultation id ${consultation.id}`, { error: error?.message || error }, 'server-action', null);
    }
    ...
    try {
      ...
      sails.config.customLogger.log('info', `Consultation status updated to closed in database for consultation id ${consultation.id}`, null,'server-action', null);
    } catch (error) {
      sails.config.customLogger.log('error', `Error updating consultation status in database for consultation id ${consultation.id}`, { consultationId: consultation.id, error: error?.message || error }, 'server-action', null);
    }
    ...
    try {
      ...
      sails.config.customLogger.log('info', `Messages updated with consultation closed timestamps for consultation id ${consultation.id}`, null,'server-action', null);
    } catch (error) {
      sails.config.customLogger.log('error', `Error updating messages with consultation closed timestamps for consultation id ${consultation.id}`, { error: error?.message || error }, 'server-action', null);
    }
    ...
    sails.config.customLogger.log('info', `Consultation closed event emitted for consultation id ${consultation.id}`, null, 'server-action', null);
  },
  ...
  getConsultationReport(consultation) {
    ...
    sails.config.customLogger.log('info', `Consultation report generated for consultation id ${consultation.id}`, null, 'message', null);
  ...
  async sendPatientReadyToQueue(consultation, queue) {
    try {
      ...
      sails.config.customLogger.log('verbose', `Sending patient ready notification for consultation id ${consultation.id} to queue ${queue.id}`, null, 'server-action', null);
      for (const doctor of doctors) {
        await Consultation.sendPatientReadyToDoctor(consultation, doctor);
        sails.config.customLogger.log('verbose', `Patient ready notification sent to doctor ${doctor.id} for consultation ${consultation.id}`,  null, 'message', null);
      }
    } catch (error) {
      sails.config.customLogger.log('error', `Error sending patient ready notifications for consultation id ${consultation.id}`, { error: error?.message || error }, 'server-action', null);
      throw error;
    }
  },
  ...
  async sendPatientReadyToDoctor(consultation, doctor) {
    ...
    if (doctor && doctor.enableNotif && doctor.notifPhoneNumber) {
      ...
      if (doctor.messageService === '1') {
        ...
        if (doctorLanguage) {
          ...
          if (template && template.sid) {
            ...
            if (twilioTemplatedId) {
              ...
              sails.config.customLogger.log('info', `WhatsApp SMS sent to doctor ${doctorId} for consultation ${consultation.id}`, null, 'server-action', null);
            } else {
              sails.config.customLogger.log('warn', `Template id is missing for WhatsApp SMS for doctor ${doctorId}`, null, 'message', null);
            }
          } else {
            sails.config.customLogger.log('warn', `WhatsApp template not approved or not found for doctor ${doctorId}`, null, 'message', null);
          }
        }
      } else {
        await sails.helpers.sms.with({
        ...
        sails.config.customLogger.log('info', `SMS sent to doctor ${doctorId} for consultation ${consultation.id}`, null, 'message', null);
      }
    }
  },
